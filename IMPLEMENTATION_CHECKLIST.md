...
- [~] §21: включён `noUncheckedIndexedAccess` в tsconfig node/web/tests + правки индексации; дальше — следующие строгие флаги только после разборки ошибок (`exactOptionalPropertyTypes` и т.п.).
...
- [x] Включить/проверить strict TypeScript политику: базовый `@electron-toolkit/tsconfig` уже с `strict`; дополнительно явно включены `noImplicitAny` и `noUncheckedIndexedAccess` в `tsconfig.node.json`, `tsconfig.web.json`, `tsconfig.tests.json`.
