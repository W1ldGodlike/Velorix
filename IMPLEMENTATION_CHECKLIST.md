...
- [~] §7.2/§7.3/§7.4: обработка — trim/crop/rotate/flip/scale/FPS/bitrate/presets/2-pass + audio gain `-filter:a volume`, strip metadata/chapters, subtitle copy/mov_text, видеофильтры `hqdn3d`/`unsharp` (3 ступени) есть; дальше — `eq`/расширенный audio filter chain, HW encode, batch и связка download→encode.
...
- [~] Filters: §7.2 — `hqdn3d` (denoise) и `unsharp` (sharpen), по 3 ступени каждая, через белый список (`EXPORT_VIDEO_DENOISE_OPTIONS`/`EXPORT_VIDEO_SHARPEN_OPTIONS`); порядок `-vf`: transform → crop → denoise → sharpen → scale → fps. `eq`/`deband`/`lut3d` и т.п. — позже.
