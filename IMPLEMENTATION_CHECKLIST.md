...
- [ ] Нет `bin/` с движками и механизма загрузки/проверки хешей.
...
- 2026-05-09: добавлены короткие архитектурные комментарии в `main`, `preload`, `settings-store` и renderer bootstrap. Дальше комментировать публичные API и сложную логику по мере появления модулей, без комментариев к очевидным строкам.

## Ближайший TODO спринта

- [ ] §3: завести слой путей приложения (`resources`, `userData`, `bin`, dev/prod).
- [ ] §3: создать модели состояния движков: `missing`, `checking`, `ready`, `error`.
- [ ] §3: сделать main-сервис для проверки `ffmpeg`, `ffprobe`, `yt-dlp` через `--version`.
- [ ] §3: добавить whitelist IPC для статуса движков.
...
- [ ] Добавить доменные сервисы: `engines`, `ffmpegRunner`, `ffprobe`, `ytdlp`, `presets`, `sessions`, `logs`.
...
- [ ] Создать структуру `bin/` для dev/prod.
- [ ] Определить имена бинарников по платформам: `ffmpeg`, `ffprobe`, `yt-dlp`.
- [ ] Реализовать поиск bundled бинарников в `process.resourcesPath`.
- [ ] Реализовать fallback на пользовательские пути из настроек.
- [ ] Реализовать проверку `--version` для каждого движка.
- [ ] Реализовать статус движков в main: отсутствует / проверяется / готов / ошибка.
- [ ] Реализовать IPC: получить статус движков.
...
- [ ] UI: показать версии движков в статусбаре.
...
- [~] Комментарии на русском для публичных API и сложной логики: базовые комментарии добавлены, продолжать при реализации модулей.
