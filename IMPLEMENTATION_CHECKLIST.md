...
- [~] §7.2/§7.3/§7.4: обработка — trim/crop/rotate/flip/scale/FPS/bitrate/presets/2-pass + audio gain/normalize, strip metadata/chapters, subtitle copy/mov_text, видеофильтры `hqdn3d`/`deband`/`lut3d` (bundled `.cube`)/`unsharp`/`eq` есть; дальше — прочие фильтры, HW encode, batch (связка download→encode уже есть).
...
- [~] Filters: §7.2 — `hqdn3d` (denoise), `deband`, `unsharp` (sharpen) и `eq` цветокор-пресеты через белый список; порядок `-vf`: transform → crop → denoise → deband → sharpen → eq → scale → fps. `lut3d` и т.п. — позже.
