...
- [~] §1.1/§4.A/§6: UI — v0-подход начат по [`docs/UX_REFERENCE_V0.md`](docs/UX_REFERENCE_V0.md): тёмная монтажная палитра, компактный topbar, вкладки Editor/Downloads, FFmpeg-настройки в правой панели; окно загрузок переведено на table/log/right-settings layout; дальше — единые иконки и сохранение раскрытых секций.
...
- [~] Сделать панели настроек кодирования сбоку/снизу, сворачиваемые: FFmpeg-панель справа уже есть, дальше нужна настройка сохранения раскрытых секций и полировка всех экранов.
- [~] Сформировать отдельное окно менеджера загрузок в едином стиле: data HTML окно переведено ближе к v0 (topbar, таблица, нижние log/history панели, right settings rail); финальная полировка/иконки — дальше.
...
- [~] Отдельное BrowserWindow под менеджер: HTML + очередь/лог/история/settings yt-dlp §6; v0-компоновка table + нижний log/history + правый settings rail начата, без React.
