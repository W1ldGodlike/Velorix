...
- [~] §21: явно зафиксированы `noImplicitAny`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `noPropertyAccessFromIndexSignature`, `useUnknownInCatchVariables` в tsconfig node/web/tests; bracket-доступ для неструктурных JSON/DOM; дальше — продуктовый фокус §6 (ошибки загрузок) или §7 без расширения строгости TS без нужды.
