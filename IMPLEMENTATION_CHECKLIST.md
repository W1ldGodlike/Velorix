...
- [~] §19/§3/§17: release/security hardening — CI (кэш `bin/` + restore-keys, prepare, verify, build, `pack:dir`), `check:release`, `release:win`, `engines:prepare:win:force` / `FLUXALLOY_ENGINES_FORCE`, `engines:report-hashes`, NSIS+portable+zip; дальше — ручной smoke артефактов, exe-хеши в JSON, подпись.
