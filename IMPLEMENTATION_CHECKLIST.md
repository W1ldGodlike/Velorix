...
- [~] §19/§3/§17: release/security hardening — CI (concurrency cancel-in-progress, кэш `bin/` + restore-keys, prepare, verify, build, `pack:dir`), `check:release`, `release:win` / `release:win:force`, `engines:prepare:win:force`, `engines:report-hashes`, NSIS+portable+zip; дальше — ручной smoke артефактов, exe-хеши в JSON, подпись.
