...
- [~] §6.1/§6.4: yt-dlp — очередь/лог/история/retry/пауза; прогресс (+ resume byte…) + `extractYtdlpOutputPath`; классификация (+ skip: нет форматов / unsupported URL…); дальше — редкие строки по реальным логам.
