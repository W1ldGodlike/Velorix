...
- [x] §6.1: колонка «Прогресс» в очереди yt-dlp — `% · скорость · ETA` из stdout/stderr (`parseYtdlpDownloadProgressLine` / `formatYtdlpProgressCell`).
- [~] §6.2: шаблон `-o`, пресеты `-f`, плейлист, аудио `-x`, субтитры, cookies, whitelist impersonate, лимит скорости и ретраи; §6.3 — доп. argv + превью + справочник; дальше расширенные retry-режимы и пр.
...
- [~] Ограничения скорости/ретраи (`--limit-rate`, `--retries`; без fragment-retries и профилей retry пока).
