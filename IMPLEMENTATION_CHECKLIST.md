...
- [~] §19/§3/§17: release/security — CI (shallow checkout, кэш `bin/`, prepare→verify→build→`pack:dir`, `permissions`+concurrency), `engines:doctor`, `release:win*`/`check:release`, env таймауты, verify/report/prepare `--help`, NSIS+portable+zip, CSC-доки в RELEASE; дальше — ручной smoke артефактов, exe-хеши в JSON, подпись.
