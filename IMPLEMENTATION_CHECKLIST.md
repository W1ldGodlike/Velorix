...
- [~] §6.1/§4.A: вкладка `Загрузки` — React workspace подключён к живой очереди; добавлены URL-band, queue table, summary cards, status filter chips, progress bars, file/folder actions, удаление строк/очистка готовых/очистка очереди, встроенный rail основных настроек yt-dlp, история с действиями и pop-out; дальше — перенос log/редких settings во вкладку и ручная матрица Win 125–200 %.
...
- [~] Сформировать вкладку `Загрузки` в едином workspace: React слой уже показывает URL-band + живую queue table + summary cards + filter chips + progress bars + управление строками/очисткой + встроенный rail основных yt-dlp настроек + историю + pop-out; старое data HTML окно остаётся pop-out с log/редкими settings; дальше — перенос log и ручная DPI-матрица.
