...
- [~] §1.1/§4.A/§6: UI — окно загрузок: icon-only очередь/история, persist ffprobe, v0-колонки, `queue.json`; после плотности layout (rail, log, topbar, 24px icons, моноширинный URL, focus-visible) — эвристики format/size и полировка инспектора §9.
...
- [~] Таблица: имя (хост+путь), ссылка; колонки Формат/Размер/Прогресс/Скорость/ETA; `progress` суммарная строка; действия icon-only; очередь дополнительно в `queue.json` §4.1; дальше — расширение эвристик format/size.
