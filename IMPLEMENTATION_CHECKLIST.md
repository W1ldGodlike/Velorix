...
- [~] §6.1/§6.2/§6.4: менеджер yt-dlp — очередь, сводка/фильтр/очистка строк, прогресс, каталог, cookies, impersonate, лимит/ретраи, retry строки, история + фильтр/повтор URL/открытие файла/открытие в обработчике, экспорт видимого лога; статус ошибки — ERROR:, fallback stderr, код/сигнал; дальше — расширенный retry/политика ошибок.
...
- [~] Обработка ошибок: приоритет текста `ERROR:`; иначе последняя строка stderr; явное завершение по сигналу ОС; `--retries`/`--fragment-retries` yt-dlp + повторы очереди §6.4 + ручной retry строки; расширенная политика retry — позже.
