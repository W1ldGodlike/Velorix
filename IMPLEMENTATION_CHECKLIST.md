...
- [~] §19/§3/§17: release/security hardening — CI (`permissions: read`, concurrency, кэш `bin/`, prepare, verify+лог версий, build, `pack:dir`), `FLUXALLOY_ENGINE_DOWNLOAD_TIMEOUT_MS`, `check:release`, `release:win` / `release:win:force`, `engines:prepare:win:force`, `engines:report-hashes` (`--versions`), NSIS+portable+zip; дальше — ручной smoke артефактов, exe-хеши в JSON, подпись.
