...
- [~] §6.1/§6.2: менеджер yt-dlp — очередь, прогресс, каталог, cookies, impersonate, лимит/`--retries`, профили повтора строки §6.4, **базовая история загрузок** (`history.json` + панель в окне); дальше fragment-retries и фильтры истории.
...
- [~] §6.4: история загрузок yt-dlp — persist `userData/downloads/history.json`, IPC и таблица в окне; **фильтры / повтор из списка** — позже.
