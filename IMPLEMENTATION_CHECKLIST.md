...
- [~] §6.1/§6.2/§6.4: менеджер yt-dlp — очередь, прогресс, каталог, cookies, impersonate, лимит/`--retries`, профили повтора, базовая история; дальше fragment-retries, фильтры/повтор из истории и обработка ошибок.
- [~] §6.3: экспертный режим — доп. argv + whitelist/blacklist + справочник; дальше сгруппировать параметры в сворачиваемые секции.
...
- [~] §17/§18: меню диагностических папок и базовый `logger-service` (`main.log` + ротация + crash handler + renderer-канал); дальше — диалог ошибки, Support ZIP и логи stdout/stderr движков.
- [~] §21: подключён Vitest + 64 теста для парсеров yt-dlp; найден и закрыт обход whitelist `-P`. Дальше — IPC contracts/shared models и тесты на ffmpeg-прогресс/output-pattern.
...
- [~] Таблица: имя (хост+путь), ссылка, прогресс (% · скорость · ETA из типичных строк `[download]` stdout/stderr), статус §6.1; нестандартные режимы вывода yt-dlp — позже.
...
- [~] Ограничения скорости/ретраи (`--limit-rate`, `--retries`); профили **повтора строки очереди** при ненулевом exit (`off`/`light`/`normal`); fragment-retries — позже.
...
- [~] Парсинг прогресса yt-dlp: процент + скорость + ETA из строк stdout/stderr (таблица §6.1 / лог §6.4).
...
- [~] Меню утилит: подменю «Инструменты → Открыть папку…» с whitelist каталогов (`diagnostics-paths`).
- [ ] Извлечь кадры.
...
- [~] Unit tests для чистых модулей: `tests/main/*` — `ytdlp-extra-args`, `ytdlp-progress-parser`, `ytdlp-queue-retry`, `ytdlp-download-history` (64 теста). Дальше — output-pattern, ffmpeg-progress, settings-store парсеры.
