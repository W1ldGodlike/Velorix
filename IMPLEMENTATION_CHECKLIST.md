...
- [~] §6.1/§6.4: yt-dlp — очередь, лог, история, авто-preview, профили повтора, маркеры stderr + классификация + коды 2/100/101, пауза SIGSTOP/SIGCONT, прогресс total/playlist/fragment/`N of M videos`, best-effort пути из лога (`extractYtdlpOutputPath`: в т.ч. «…; Destination:», произвольный `[PP] Destination:`, EmbedThumbnail/Adding metadata/Moving/Concat); дальше редкие строки прогресса и эвристики ошибок по фактам из логов.
...
- [~] Парсинг прогресса yt-dlp: процент + скорость + ETA + `fragment X of Y` + `Total progress:` + `Downloading video|item X of Y` + вариант `N of M videos`; прочие редкие строки — по мере заметок.
