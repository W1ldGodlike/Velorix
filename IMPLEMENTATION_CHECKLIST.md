...
- [~] §6.1/§6.4: yt-dlp workspace почти основной: очередь/лог/история/retry/pause/settings/catalog/cookies/network встроены; snapshot очереди рассылается во вкладку и pop-out; основные mutating IPC возвращают явный `{ok,error}`, без тихих no-op; дальше — редкие settings, редкие шаблоны логов и ручная DPI-матрица.
...
- [ ] Убрать все литералы интерфейса в локализацию; быстрые editor/downloads labels уже подчистить на русском, но `locales/**` ещё нет.
