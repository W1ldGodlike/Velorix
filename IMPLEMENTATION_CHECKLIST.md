...
- [~] §6.1/§6.2/§6.4: менеджер yt-dlp — очередь, сводка/фильтр/очистка строк, прогресс, каталог, cookies, impersonate, лимит/ретраи, retry строки, история + фильтр/повтор URL/открытие файла, экспорт видимого лога; дальше обработка ошибок и сценарий «открыть в обработчике».
...
- [~] «Скачать и открыть»: готовый файл можно открыть/показать в папке из очереди и истории; передача в обработчик FluxAlloy — позже.
- [ ] «Скачать и сразу обработать».
- [~] Обработка ошибок: краткий текст из последней строки `ERROR:` в статусе + `--retries`/`--fragment-retries` yt-dlp + повторы очереди §6.4 + ручный retry строки; расширенная политика retry — позже.
...
- [~] Unit tests для чистых модулей: `tests/main/*` — `ytdlp-extra-args`, `ytdlp-progress-parser`, `ytdlp-queue-retry`, `ytdlp-download-history` (append/read/clear), `ytdlp-download-options` (filename/output-pattern/rate-limit/retries), `downloads-queue` (cleanup), `settings-store` (yt-dlp persisted fields), `ffmpeg-export-service` (progress helpers/presets), `external-process-log` (sanitize/format), `support-bundle` (ZIP structure/log inclusion/prune). Дальше — IPC contracts/shared models.
...
- [~] Приёмочный сценарий: URL -> yt-dlp -> открыть/показать файл; передача в обработчик FluxAlloy — позже.
