...
- [~] §1.1/§4.A/§9: редактор — HiDPR/topbar/SVG/focus; **`aria-current`** **«Редактор»**; **`.app-url-hint`** + **`aria-describedby`** (быстрая yt-dlp); **превью ffmpeg**: **`pre.app-export-preview-pre`** → **`aria-describedby`** на **`#exportCommandPreviewHint`**; **MediaProbePanel** caption; **ffmpeg•yt-dlp**; seek/снап; **`ffmpegSettingsRailOpen`**; дальше — multi-monitor DPI, Dolby/side_data §9.
