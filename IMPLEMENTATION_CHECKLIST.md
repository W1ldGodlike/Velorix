...
- [~] Проверить адаптивность и DPI: окно yt-dlp + главное + инспектор — общий helper `window-hidpi` для `displayMatching`/`min*`; renderer — точечный @media для `app-topbar`/`app-toolbar`/`app-icon-btn` и **вкладки «Загрузки»** (`app-downloads-*`); полная ручная матрица 100–200% — позже.
