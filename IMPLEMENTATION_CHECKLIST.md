...
- [~] §6.1/§6.4: вкладка «Загрузки» v0 — таблица / rail / **история→лог**; **pop-out** — те же `emitInlineStrokeSvg` + **`cmd-icon-leading`** (лента URL, тулбар очереди, история/лог, rail cookies/каталог/сохранить/сброс шаблона); **осталось: ручная DPI-матрица 100–200%**.
- [~] §6.3: argv + whitelist + preview; pop-out: **чекбоксы rail «Формат»** vs `PillSwitch` на вкладке — выровнять; длинный справочник в pop-out.
