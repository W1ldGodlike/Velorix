...
- [~] §1.1/§4.A/§6: UI — окно загрузок: icon-only очередь + история; persist ffprobe `details`; таблица v0-колонок; живую очередь пишем в `downloads/queue.json` (восстановление + сброс «Загрузка…» при старте); дальше — плотность layout и эвристики format/size.
...
- [~] Сформировать отдельное окно менеджера загрузок в едином стиле: data HTML ближе к v0 (topbar, очередь v0-колонки + история icon-only, log/history, settings rail); живая очередь в `downloads/queue.json` §4.1; дальше — плотность layout.
