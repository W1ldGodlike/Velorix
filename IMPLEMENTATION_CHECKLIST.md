...
- [~] §6.1/§4.A: вкладка `Загрузки` — React workspace подключён к живой очереди; добавлены URL-band, queue table, summary cards, status filter chips, progress bars, file/folder actions, встроенный rail основных настроек yt-dlp и pop-out; дальше — перенос log/history/редких settings во вкладку и ручная матрица Win 125–200 %.
...
- [~] Сформировать вкладку `Загрузки` в едином workspace: React слой уже показывает URL-band + живую queue table + summary cards + filter chips + progress bars + встроенный rail основных yt-dlp настроек + pop-out; старое data HTML окно остаётся pop-out с log/history/редкими settings; дальше — перенос log/history и ручная DPI-матрица.
