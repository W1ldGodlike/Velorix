...
- [~] §1.1/§4.A/§6: UI — окно загрузок: icon-only очередь + история (SVG в `act-icons`); persist `details` инспектора под превью (`probe*` в `mainWindowUiPanels`); дальше — колонки v0 (size/ETA/speed) и прочая полировка таблицы загрузок.
...
- [~] Сформировать отдельное окно менеджера загрузок в едином стиле: data HTML окно переведено ближе к v0 (topbar, таблица очереди и истории с icon-only действиями, нижние log/history, right settings rail с persist секций); полировка колонок очереди (size/ETA) — дальше.
...
- [~] Таблица: имя (хост+путь), ссылка, прогресс (% · скорость · ETA из типичных строк `[download]` stdout/stderr), статус и best-effort путь для open; столбец действий — icon-only (v0); отдельные колонки format/size как в референсе — позже.
