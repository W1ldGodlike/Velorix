...
- [~] Добавить единый набор иконок: топбар редактора (folder + **rotate-ccw/cw + scissors v0**, `circle-help`/загрузки из shared) + транспорт под превью + столбец действий очереди/истории yt-dlp + **тулбар окна инспектора §9** (`app-icon-btn`, shared strokes); содержимое панелей инспектора — при необходимости дальнейшее выравнивание.
