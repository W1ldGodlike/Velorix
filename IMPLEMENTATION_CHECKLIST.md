...
- [~] §6.1/§6.4: вкладка «Загрузки» v0 — таблица / rail / **история→лог**; **pop-out** — `emitInlineStrokeSvg` + **`cmd-icon-leading`** + **pill rail** (плейлист/аудио/открыть после успеха); **осталось: ручная DPI-матрица 100–200%**.
- [~] §6.3: argv + whitelist + preview; pop-out **узкий ≤960px**: rail **под** очередью + scroll + **«К настройкам»** (`#dl-ytdlp-settings-rail`); длинный справочник токенов; редкие поля.
