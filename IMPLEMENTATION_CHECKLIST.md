...
- [~] §6.1/§4.A: вкладка `Загрузки` — React workspace подключён к живой очереди; добавлены URL-band, queue table, summary cards, status filter chips, progress bars, file/folder actions и pop-out; дальше — перенос rail/log/history/settings во вкладку и ручная матрица Win 125–200 %.
...
- [~] Сформировать вкладку `Загрузки` в едином workspace: React слой уже показывает URL-band + живую queue table + summary cards + filter chips + progress bars + pop-out; старое data HTML окно остаётся pop-out с log/history/settings rail; дальше — перенос rail/log/history/settings во вкладку и ручная DPI-матрица.
