...
- [~] §7: превью + таймлайн + экспорт MP4/MKV/MOV + действия открыть файл/папку результата + снимок кадра §7.6 + ffprobe под превью; отдельное окно инспектора §9 — позже.
...
- [~] Приёмочный сценарий: открыть файл -> preview -> экспорт -> открыть файл/показать в папке; ручной e2e smoke в packaged-сборке ещё нужен.
