...
- [~] §19/§3/§17: release/security — CI (shallow checkout, кэш Electron/electron-builder/`bin/`, prepare→verify→build→`pack:dir`, `permissions`+concurrency), `check:trusted-hashes` в `npm run check`, `engines:doctor`, `release:win*`/`check:release`, env таймауты, `--help` у скриптов движков, NSIS+portable+zip, CSC в RELEASE; дальше — ручной smoke артефактов, заполнить exe-хеши, подпись.
