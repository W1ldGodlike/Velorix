...
- [~] §1.1/§4.A/§6: UI — topbar главного окна: lucide-style иконки; раскрытие FFmpeg/быстрого yt-dlp и секций окна загрузок persist в `settings.json`; дальше — icon-only действия очереди загрузок, collapsible инспектора под превью, полировка.
...
- [~] Добавить единый набор иконок: топбар редактора — inline SVG lucide-подход; загрузки/инспектор — позже выровнять.
