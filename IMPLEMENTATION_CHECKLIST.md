...
- [~] §9: отдельное окно инспектора §363; опционально позже поля ffprobe про цвет/HDR (pix_fmt/SAR/DAR уже в таблице дорожек).
...
- [x] Drag-and-Drop URL/текста на поле ввода и на свободную область окна загрузок (не перехватываем drop на `textarea`/`select`/текстовых `input` параметров §6.2).
...
- [x] Таблица дорожек под превью (`tags`, битрейт/`disposition`, видео `pix_fmt`/SAR/DAR, контекстное меню; отдельное окно — §363).
