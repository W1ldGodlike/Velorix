...
- [~] §6.1/§6.2/§6.4: менеджер yt-dlp — очередь, прогресс, каталог, cookies, impersonate, лимит/`--retries`, профили повтора, история + повтор URL; дальше fragment-retries, фильтры истории и обработка ошибок.
...
- [x] История загрузок (файл `downloads/history.json`, атомарная запись temp+rename после yt-dlp, IPC, UI, повтор URL в очередь).
