...
- [~] §1.1/§4.A/§6/§9: окно загрузок — lucide/topbar/тема/HiDPI/v0-stack log/history; инспектор — тулбар `app-icon-btn` + `lucide-downloads-icons` (открыть/refresh-cw/тема); дальше — системная тема OS, DPI-матрица, полировка таблицы инспектора.
...
- [~] Довести палитру, типографику, отступы, радиусы и focus-состояния на всех экранах: главный renderer и downloads (токены `--fa-*`/`focus-ring` в data HTML) сближены; светлая/тёмная тема второго окна синхронна с настройкой приложения; инспектор: тулбар на общих lucide/`app-icon-btn`, тело панелей — дальше полировка.
...
- [~] Добавить единый набор иконок: топбар редактора + столбец действий очереди/истории yt-dlp + **тулбар окна инспектора §9** (`app-icon-btn`, shared strokes); содержимое панелей инспектора — при необходимости дальнейшее выравнивание.
