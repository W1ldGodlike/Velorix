...
- [~] Аудио кодек: AAC, **копировать дорожку (copy)**, **PCM s16le**, **Vorbis (libvorbis, MKV-only)**, **Opus (libopus, MKV-only)**, **FLAC (MKV-only)** или без аудио; **громкость аудио** через `-filter:a volume=NdB` (`ffmpegExportAudioGainDb`, шаг 3 дБ, диапазон −24…+24); выбор другого кодека — позже.
