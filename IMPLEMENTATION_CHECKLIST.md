...
- [~] §7.2/§7.3/§7.4: обработка — trim/crop/rotate/flip/scale/FPS/bitrate/presets/2-pass + audio gain/normalize, strip metadata/chapters, subtitle copy/mov_text, видеофильтры `hqdn3d`/`deband`/`unsharp`/`eq` есть; дальше — `lut3d`/прочие фильтры, HW encode, batch (связка download→encode уже есть).
