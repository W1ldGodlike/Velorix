...
- [~] §8: терминал/CLI — вкладка Electron, allowlist `ffmpeg`/`ffprobe`/`yt-dlp`, `execFile` без shell, PATH с папкой выбранного движка, подсказки из `Data/ffmpeg_commands.json` + `Data/ytdlp_commands.json`, история сессии и вывод готовы; дальше — подстановка текущего файла/превью и логирование команд/результата в диагностику.
