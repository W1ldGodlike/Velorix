...
- [~] §1.1/§2.2/§5: локализация, состояние renderer, контрасты/focus/token audit и ручная DPI/multi-monitor матрица; подход по состоянию зафиксирован в коде через выделенный hook `useMainWindowUiPanels` (единая гидрация + persist), дальше — расширить паттерн на остальные крупные UI-срезы и закрыть `locales/**`.
