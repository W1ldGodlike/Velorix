...
- [~] §19/§3/§17: release/security — CI и локальные сценарии выровнены (strict `trusted_hashes` в Actions, shallow+caches, **`workflow_dispatch`**, prepare→`engines:doctor`→build→`pack:dir`, `check:release`/`release:win*`, таймауты, NSIS+portable+zip, README/RELEASE/bin/`electron-builder`/`--help`). **Дальше вручную:** непустые SHA в `trusted_hashes.json`, при необходимости `FLUXALLOY_ENGINES_STRICT` в CI, смоук NSIS/portable/zip, подпись Windows.
