...
- [~] §1.1/§4.A/§6: UI — окно загрузок: lucide shared + topbar cluster/mост; палитра через `--fa-*` (как dark `base.css`), `focus-visible` 2px/`--fa-focus-ring`, шрифтовой стек редактора; дальше — опционально следовать системной теме в окне загрузок через IPC и §9 инспектор.
...
- [~] Довести палитру, типографику, отступы, радиусы и focus-состояния на всех экранах: главный renderer и downloads (токены `--fa-*`/`focus-ring` в data HTML) сближены; светлая тема второго окна — позже; inspector ещё требует полировки.
