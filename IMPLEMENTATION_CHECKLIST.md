...
- [~] §6.2: шаблон `-o` + пресеты `-f` в UI загрузок и `settings.json`; дальше аудио-only, плейлист, cookies и произвольные аргументы §6.3.
- [x] §7.1: в статусе экспорта — `speed=` из stderr ffmpeg + разбор прогресса по `\r`/`\n`.
- [ ] §7.2: первый системный пресет экспорта (или CRF/preset в UI) поверх фиксированного libx264/aac.
...
- [~] Таблица: имя (хост+путь), ссылка, прогресс (% · скорость · ETA из типичных строк `[download]` stderr), статус §6.1; нестандартные режимы вывода yt-dlp — позже.
...
- [~] Парсинг прогресса yt-dlp: процент + скорость + ETA из строк stderr (таблица §6.1 / лог §6.4).
