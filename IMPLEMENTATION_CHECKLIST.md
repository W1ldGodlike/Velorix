...
- [~] §7: превью + таймлайн + экспорт MP4/MKV/MOV + снимок кадра §7.6 + ffprobe под превью; отдельное окно инспектора §9 — позже.
- [~] §7.2/§20: системные пресеты libx264 и базовый контейнер/формат есть; дальше пользовательские пресеты и расширенные параметры кодирования.
...
- [~] Контейнер/формат: save dialog поддерживает MP4/MKV/MOV и добавляет расширение по умолчанию; отдельная панель выбора — позже.
- [ ] Видео кодек.
- [ ] Аудио кодек.
- [ ] Bitrate/CRF/quality.
- [ ] FPS.
...
- [~] Unit tests для чистых модулей: `tests/main/*` — `ytdlp-extra-args`, `ytdlp-progress-parser`, `ytdlp-queue-retry`, `ytdlp-download-history` (append/read/clear), `ytdlp-download-options` (filename/output-pattern/rate-limit/retries), `downloads-queue` (cleanup), `settings-store` (yt-dlp persisted fields), `ffmpeg-export-service` (progress helpers/presets/container extension), `external-process-log` (sanitize/format), `support-bundle` (ZIP structure/log inclusion/prune). Дальше — IPC contracts/shared models.
...
- [~] Приёмочный сценарий: URL -> yt-dlp -> открыть/показать файл -> отправить в обработчик FluxAlloy; автоматическая обработка — позже.
