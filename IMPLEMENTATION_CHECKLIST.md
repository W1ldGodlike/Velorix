...
- [~] §7: превью + таймлайн + экспорт MP4/MKV/MOV + действия открыть файл/папку результата/вернуть экспорт в превью + снимок кадра §7.6 + ffprobe под превью; отдельное окно инспектора §9 — позже.
- [~] §7.2/§20: системные пресеты libx264, persisted контейнер/формат, CRF или video bitrate, AAC bitrate, FPS и scale preset есть; дальше пользовательские пресеты и расширенные параметры кодирования.
...
- [~] Приёмочный сценарий: открыть файл -> preview -> экспорт -> открыть файл/показать в папке/вернуть в preview; ручной e2e smoke в packaged-сборке ещё нужен.
