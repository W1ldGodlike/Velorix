...
- [~] §7: превью + таймлайн (seek) + краткий ffprobe-текст после grant; без маркеров in/out и полноценного инспектора §9.
...
4. [~] Главное окно обработки: файл/DnD, таймлайн и ffprobe-сводка; панели ffmpeg и полный URL-пайплайн — дальше.
5. [~] Окно менеджера yt-dlp: очередь + последовательный spawn yt-dlp; без отдельной строки «Старт», полного лога и парсинга скорости.
...
- [~] Структура `bin/` (bundled + `userData/bin`); установка через UI-загрузку на Win; в репозитории бинарники не коммитятся.
...
- [~] Отдельное BrowserWindow под менеджер: пока `data:`-заглушка без React/очереди §6.
...
- [~] Последний открытый локальный файл (`lastOpenedSourcePath`) + мягкий restore превью при старте; без полного session.json.
...
- [~] Парсинг прогресса yt-dlp: процент из строк stderr (не полный лог UI).
- [ ] Лог stdout/stderr.
...
- [~] Таймлайн: базовый scrub; маркеры in/out и доменная модель монтажа — позже.
- [ ] Маркеры in/out.
...
- [~] Запуск ffprobe: в main только для уже grant-путей (IPC probe), краткая строка под превью; отдельное окно/таблицы дорожек — позже.
