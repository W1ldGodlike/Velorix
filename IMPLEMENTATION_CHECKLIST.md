...
- [~] §6.1/§6.4: yt-dlp — очередь/лог/история/retry/пауза; прогресс + `extractYtdlpOutputPath` (+ remux, `→`/`-> Destination`, SubtitlesConvertor); классификация stderr (+ signature/rate limit · skip DRM…); дальше — редкие строки по реальным логам.
