...
- [~] §1.1/§4.A/§6: UI — окно загрузок: lucide/topbar/`--fa-*`/тема; HiDPI: `@media (min-resolution/dppx)` + стартовый размер/`min*` окна от `scaleFactor` монитора (`screen.getDisplayMatching`); дальше — системная тема/OS, ручная проверка 100/125/150/200%, полировка §9 инспектора.
...
- [~] Проверить адаптивность и DPI: окно yt-dlp — CSS по `resolution`/`-webkit-device-pixel-ratio` и `minWidth`/`minHeight` от монитора; главное окно и полная матрица масштабов — позже.
