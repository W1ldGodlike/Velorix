...
- [~] §6.1/§6.4: yt-dlp — очередь/лог/история/retry/пауза; прогресс + `extractYtdlpOutputPath` (+ remux `into …`, `→ Destination`); классификация stderr (408/premature close, skip: live ended/premiere…); дальше — редкие строки по реальным логам.
- [~] §6.3: argv whitelist + справочник + превью с draft/override `-o`; при необходимости расширить overlay под редкие поля формы.
...
- [~] Парсинг прогресса yt-dlp: процент + скорость + ETA + `fragment X of Y` + `(frag N/M)` без процентов в строке + `Total progress:` + `Downloading video|item X of Y` + вариант `N of M videos` + `Sleeping … seconds` / `Waiting for reconnect` / прочие `[download] Waiting for …`; прочие редкие строки — по мере заметок.
