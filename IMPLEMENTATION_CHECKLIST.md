...
- [~] §1.1/§4.A/§6: UI — окно загрузок: icon-only действия строки очереди (SVG) + снимок с `isActiveRunner`/`ytdlpPaused`; дальше — иконки таблицы истории, persist collapsible инспектора под превью, полировка колонок v0 (size/ETA).
...
- [~] Сформировать отдельное окно менеджера загрузок в едином стиле: data HTML окно переведено ближе к v0 (topbar, таблица очереди с icon-only действиями, нижние log/history, right settings rail с persist секций); полировка истории/колонок — дальше.
...
- [~] Добавить единый набор иконок: топбар редактора + столбец действий очереди yt-dlp (inline SVG); таблица истории загрузок и инспектор — позже выровнять.
...
- [~] Сохранять раскрытые панели: главное окно (`mainWindowUiPanels`) + окно yt-dlp (`downloadsWindowUiPanels`); блоки инспектора/ffprobe под превью пока без persist.
