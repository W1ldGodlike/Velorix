...
- [~] §6.1/§6.2/§6.4: менеджер yt-dlp — очередь, прогресс, каталог, cookies, impersonate, лимит/`--retries`, `--fragment-retries`, профили повтора, retry строки, история + фильтр/повтор URL, экспорт видимого лога; дальше обработка ошибок.
...
- [x] История загрузок (файл `downloads/history.json`, атомарная запись temp+rename после yt-dlp, IPC, UI, фильтр по исходу, повтор URL в очередь).
