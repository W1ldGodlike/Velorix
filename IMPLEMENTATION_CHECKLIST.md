...
- [x] Vitest smoke для `probeMediaFile`: нет ffprobe в каталогах и override на исполняемый не-ffprobe → `ok: false` без падения (`tests/main/ffprobe-probe-media.integration.test.ts`); невалидный JSON stdout — отдельный файл с `vi.mock('child_process')` (`ffprobe-probe-media-json-mock`).
