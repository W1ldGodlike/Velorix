...
- [~] §9: опционально позже расширенные поля ffprobe (цвет/HDR); pix_fmt/SAR/DAR уже в таблице дорожек.
...
- [x] Таблица дорожек под превью и в отдельном окне (`tags`, битрейт/`disposition`, видео `pix_fmt`/SAR/DAR, контекстное меню).
