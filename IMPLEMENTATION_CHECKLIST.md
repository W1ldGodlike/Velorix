...
- [~] §6.1/§6.4: вкладка «Загрузки» v0 — таблица / rail / история→лог; **прогресс: полоска + `%` справа**; **URL-band**: primary + `plus`/`play`/`ban`/`trash` из **`QUEUE_ROW_ACTION_ICONS`** + band `clipboard`/`popOutWindow`; HiDPI **120 / 168 / 192**; **узкая ширина**: rail под колонкой; **осталось: ручная DPI-матрица 100–200%**.
