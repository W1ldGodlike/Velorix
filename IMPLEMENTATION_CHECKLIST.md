...
- [~] §1.1/§4.A/§6: UI — окно загрузок: icon-only очередь/история, persist ffprobe, v0-колонки, `queue.json`, плотный layout; дальше — выравнивание иконок с главным окном, сохранённые раскрытия секций (polish), полировка инспектора §9.
...
- [~] Таблица: имя (хост+путь), ссылка; колонки Формат/Размер/Прогресс/Скорость/ETA; `progress` суммарная строка; действия icon-only; очередь дополнительно в `queue.json` §4.1; колонки format/size дополняются `parseYtdlpQueueFormatHint` / `[info]` filesize до `% of`; дальше — редкие шаблоны логов.
