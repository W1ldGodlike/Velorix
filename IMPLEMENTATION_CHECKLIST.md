...
- [~] Фильтры: история обработок фильтруется по kind/outcome/query; встроенная история загрузок — по outcome.
