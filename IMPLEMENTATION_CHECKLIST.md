...
- [~] §6.1/§4.A: вкладка `Загрузки` — базовый React workspace слой (URL-band + queue table + pop-out) подключён к живой очереди; старое окно остаётся pop-out с log/history/settings rail; дальше — перенос rail/log/history во вкладку и ручная матрица Win 125–200 %.
...
- [~] Сформировать вкладку `Загрузки` в едином workspace: базовый React слой уже показывает URL-band + queue table + pop-out; старое data HTML окно остаётся pop-out с log/history/settings rail; дальше — перенос rail/log/history во вкладку и ручная DPI-матрица.
