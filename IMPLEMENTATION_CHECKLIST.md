...
- [~] §9/§18: ffprobe/диагностика — inspector, TXT/HTML/JSON export, logs, Support ZIP есть; в сводке дорожки — расширенные видео-поля ffprobe + SAR/DAR (кроме тривиального 1:1), `matrix …°` из Display Matrix или `rot …°` из тега `rotate`; для аудио — `profile`/`sample_fmt`/N-bit PCM при `bits_per_sample`; Support ZIP `diagnostics.txt` — локали, pid/cwd/packaged, primary display (DPI); дальше — crash/e2e smoke, ещё поля отчёта при необходимости.
