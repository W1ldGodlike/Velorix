...
- [!] Для Windows dev-сборки отключено `win.signAndEditExecutable: false`, иначе `electron-builder` падает на распаковке `winCodeSign` без symlink-прав.
...
- [!] `win.signAndEditExecutable: false` временно для dev-сборки без symlink-прав; перед релизом пересмотреть.
