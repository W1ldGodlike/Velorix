...
- [~] §6.1/§6.4: yt-dlp — очередь, лог, история, авто-preview, профили повтора, маркеры stderr + классификация transient/source в статусе строки, пауза SIGSTOP/SIGCONT; дальше эвристики по коду выхода yt-dlp и парсинг прогресса (редкие форматы строк).
...
- [~] Парсинг прогресса yt-dlp: процент + скорость + ETA + строки `fragment X of Y` (таблица §6.1 / лог §6.4); редкие форматы yt-dlp — по мере заметок.
