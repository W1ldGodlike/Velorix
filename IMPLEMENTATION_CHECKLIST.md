...
- [~] §7: показ имени источника + `<video controls>` через `fluxmedia://`; без таймлайна/ffprobe-связки.
...
4. [~] Главное окно обработки: файл/DnD есть; URL, таймлайн, панели ffmpeg — дальше.
...
- [~] Меню `Файл`: «Открыть…» есть; менеджер загрузок, выход позже по ТЗ.
...
- [~] Play/pause/seek: через контролы Chromium; синхронизация с таймлайном позже.
- [ ] Таймлайн.
- [ ] Маркеры in/out.
