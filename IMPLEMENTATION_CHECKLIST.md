...
- [~] §1.1/§4.A/§9: редактор — HiDPI + топбар **ffmpeg•yt-dlp**; **`app-url-summary`/`app-url-body` @120dpi/@168dpi**; **`focus-visible` на сворачиваемой полосе быстрого yt-dlp** (`app-url-summary`); **снимок/экспорт/отмена/облако + sun/moon (`EDITOR_TOPBAR_ACTION_ICONS` / `EDITOR_THEME_ICONS`)**; seek/снап; **сворачиваемый rail FFmpeg** (persist `ffmpegSettingsRailOpen`); дальше — multi-monitor DPI, Dolby/side_data §9.
