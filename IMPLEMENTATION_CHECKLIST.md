...
- [~] §6.1/§6.2/§6.4: менеджер yt-dlp — очередь, прогресс, каталог, cookies, impersonate, лимит/`--retries`, `--fragment-retries`, профили повтора, история + фильтр/повтор URL, экспорт видимого лога; дальше обработка ошибок.
...
- [~] Обработка ошибок: краткий текст из последней строки `ERROR:` в статусе + `--retries`/`--fragment-retries` yt-dlp + повторы очереди §6.4; расширенная политика retry — позже.
