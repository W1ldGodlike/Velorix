...
- [~] §1.1/§4.A/§6: UI — окно загрузок: lucide/topbar/`--fa-*`/тема/HiDPI; v0 нижняя зона — **история + журнал операций столбиком** (лог под очередью на всю ширину rail-колонки); дальше — системная тема/OS, матрица DPI 100–200%, единые иконки §9 инспектора.
