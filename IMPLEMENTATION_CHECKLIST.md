...
- [~] §1.1/§4.A/§6/§9: загрузки lucide/layout/тема/HiDPI; live-панели; инспектор `app-topbar`/`probe*`; редактор: **полоска транспорта v0** под `<video>` + rotate/scissors в топбаре; rail pill; очередь bar+%; ffprobe context menu с клавиатуры; дальше — waveform/тонкая матрица DPI, ffprobe §9, редкие строки §6.4.
...
- [~] Таймлайн под превью (базовый range + синхрон с `<video>`); под превью — **транспорт v0** (skip/±5 с/play-pause/fullscreen/громкость, общие lucide-пути): нативные controls §7.1 сохранены; «центральный элемент» в смысле ТЗ — дальше (in/out polish, waveform).
