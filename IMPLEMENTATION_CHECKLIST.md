...
- [~] §1.1/§4.A/§6: UI — окно загрузок: lucide shared + topbar cluster/mост; палитра через `--fa-*` (light/dark как `base.css`), `data-theme` при открытии из `settings.json` + `onThemeChanged` (`mainWindowIpc.themeChanged`); дальше — опционально «системная» тема/OS и полировка §9 инспектора.
...
- [~] Сформировать отдельное окно менеджера загрузок в едином стиле: data HTML ближе к v0 (компактнее layout, таблица v0-колонки, log/history, rail); живая очередь `downloads/queue.json` §4.1; при необходимости донастроить плотность под реальные DPI.
...
- [~] Довести палитру, типографику, отступы, радиусы и focus-состояния на всех экранах: главный renderer и downloads (токены `--fa-*`/`focus-ring` в data HTML) сближены; светлая/тёмная тема второго окна синхронна с настройкой приложения; inspector ещё требует полировки.
...
- [~] Отдельное BrowserWindow под менеджер: HTML + очередь/лог/история/settings yt-dlp §6; v0-компоновка table + нижний log/history + правый settings rail; persist раскрытия секций rail/log/history; без React.
...
- [x] Стартовый размер главного окна 1920x1080 (FHD).
...
- [~] Добавить единый набор иконок: топбар редактора + столбец действий очереди и истории yt-dlp (inline SVG); отдельное окно инспектора §9 — позже выровнять с тем же стилем при необходимости.
...
- [~] Сохранять раскрытые панели: главное окно (`mainWindowUiPanels`, включая секции ffprobe под превью `probe*`) + окно yt-dlp (`downloadsWindowUiPanels`).
...
- [~] Таблица: имя (хост+путь), ссылка; колонки Формат/Размер/Прогресс/Скорость/ETA; `progress` суммарная строка; действия icon-only (общий модуль lucide-слоёв с topbar главного окна); очередь в `queue.json` §4.1; format/size из `parseYtdlpQueueFormatHint`/`[info]`; дальше — редкие шаблоны логов.
...
- [~] Лог stdout/stderr: канал IPC `fluxalloy-downloads-log` + нижняя панель журнала в v0-layout (обрезка ~240 KiB текста) + сохранение видимого лога через save dialog.
