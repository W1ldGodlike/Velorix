...
- [~] §1.1/§4.A/§6/§9: загрузки lucide/layout/тема/HiDPI (топбар ~62px + dppx); инспектор — lucide-тулбар; редактор — topbar/export preview persist + HiDPI @media topbar/toolbar/icon-btn и `focus-visible` секций FFmpeg; дальше — системная тема OS, ручная матрица DPI, полировка инспектора.
...
- [~] Проверить адаптивность и DPI: окно yt-dlp — `resolution`/dppx + `min*` окна; главный renderer/инспектор — точечный @media для `app-topbar`/`app-toolbar`/`app-icon-btn`; полная матрица 100–200% и главное окно `min*` — позже.
