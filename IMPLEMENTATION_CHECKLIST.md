...
- [~] Сводка ошибок после завершения; **drag-reorder**; economy/history/open; **retry failed** + **clear completed** + retry строки; **копировать пути (вход/выход)** / **сохранить отчёт** (TSV + колонка ошибки) / **убрать ожидающие**; открыть **готовый файл пакета в редакторе**; двойной щелчок по строке / по ячейке **«Выход»** — **исходник / результат в редакторе**; **шаблон имени выхода** (`ffmpegExportBatchOutputSuffix`); **общая папка выхода** (`ffmpegExportBatchOutputDirectory`, открыть в проводнике); авто-раскрытие панели при auto-enqueue §7.4.
