...
- [~] §1.1/§4.A/§6/§9: загрузки lucide/layout/тема/HiDPI (топбар ~62px + dppx CSS + `window-hidpi` min/дефолт); главное окно/инспектор — те же `scaleFactor`/workArea для `min*` и первичного размера; редактор — топбар v0-кластер + export preview persist + FFmpeg focus-visible; системная тема в меню/IPC; дальше — ручные прогоны 100–200%, полировка тела инспектора.
...
- [~] Добавить единый набор иконок: топбар редактора (вкл. `circle-help` для «О программе» из `DOWNLOADS_TOPBAR_CLUSTER_ICONS`) + столбец действий очереди/истории yt-dlp + **тулбар окна инспектора §9** (`app-icon-btn`, shared strokes); содержимое панелей инспектора — при необходимости дальнейшее выравнивание.
