...
- [~] §6.1/§6.4: вкладка «Загрузки» v0 — таблица / rail / история→лог; **прогресс: полоска + `%` справа**, зелёная заливка при «Готово»; **«Добавить в очередь» + «Начать загрузку»** — оба `app-btn-primary`; HiDPI **120 / 168 / 192** (`main.css`); **осталось: ручная матрица DPI**, поведение **`<1100px` без rail**.
