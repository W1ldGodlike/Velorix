...
- [~] §6.1/§6.2: менеджер yt-dlp — очередь, прогресс, каталог, cookies, impersonate, лимит/`--retries`, **профили повтора строки** §6.4; дальше история загрузок и fragment-retries.
...
- [~] Обработка ошибок: краткий текст из последней строки `ERROR:` в статусе + повторы очереди §6.4; политика retry/история файла — позже.
- [ ] История загрузок.
