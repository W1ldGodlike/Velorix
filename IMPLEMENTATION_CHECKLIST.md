...
- [~] §19/§3/§17: release/security hardening — CI (кэш `bin/` с hashFiles по скриптам verify/report/version-line + prepare + trusted_hashes), `permissions: read`, concurrency, prepare, verify+лог версий, build, `pack:dir`, `scripts/engines-exe-version-line.mjs`, `FLUXALLOY_ENGINE_DOWNLOAD_TIMEOUT_MS`, `check:release`, `release:win` / `release:win:force`, `engines:prepare:win:force`, `engines:report-hashes`/`verify` (`--help`, `--versions`), NSIS+portable+zip, `CSC_IDENTITY_AUTO_DISCOVERY` в RELEASE; дальше — ручной smoke артефактов, exe-хеши в JSON, подпись.
