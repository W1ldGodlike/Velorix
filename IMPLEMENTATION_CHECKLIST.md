...
- [~] §6.1/§6.4: v0-компоновка вкладки «Загрузки» — центр таблица, **история+живой лог под таблицей**, rail — настройки; очередь/retry/pause/settings/catalog/cookies/network встроены; snapshot очереди во вкладку и pop-out; mutating IPC с явным `{ok,error}`; ↑/↓ и «В редактор»; open после merge Windows; дальше — полировка узких высот, редкие settings/log-шаблоны, DPI-матрица.
