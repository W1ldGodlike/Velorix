...
- [~] §1.1/§4.A/§6: UI — окно загрузок: icon-only очередь + история; persist ffprobe `details`; таблица очереди — v0-колонки Формат/Размер/Прогресс/Скорость/ETA (`queue*` + парсер); дальше — плотность layout, сохранение очереди §4 и расширение эвристик format/size.
...
- [~] Сформировать отдельное окно менеджера загрузок в едином стиле: data HTML окно переведено ближе к v0 (topbar, таблица очереди v0-колонки + история icon-only, нижние log/history, right settings rail); дальше — плотность layout и persist очереди.
...
- [~] Довести палитру, типографику, отступы, радиусы и focus-состояния на всех экранах: главный renderer и downloads начаты, inspector ещё требует полировки.
...
- [~] Таблица: имя (хост+путь), ссылка; отдельные колонки Формат (`[info]` `format(s):`), Размер (`% of …`), Прогресс (полоска + первая часть объединённой строки), Скорость, ETA из парсера §6; поле `progress` сохраняет полную суммарную строку; столбец действий — icon-only (v0); дальше — persist очереди и расширение эвристик format/size.
