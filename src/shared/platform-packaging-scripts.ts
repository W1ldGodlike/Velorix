...
export function formatPlatformPackagingDiagnosticLines(): string[] {
  return [
    ...
    `engines prepare: npm run ${ENGINES_PREPARE_WIN_NPM_SCRIPT} (Windows x64; predev/prebuild:win); mac/linux — ${ENGINES_PREPARE_MAC_NPM_SCRIPT}|${ENGINES_PREPARE_LINUX_NPM_SCRIPT} (help-only bundled-first)`,
    `engines mac/linux: bin/ вручную → engines:doctor → pack:*:dir (см. ${PACKAGED_E2E_HELP_WORKFLOW_CROSSLINKS_BIN_README_PATH}, Help packaged mac/linux)`,
    ...
    formatPackagedGuiE2ePlaywrightScaffoldDiagnosticLine(),
    formatPackagedGuiE2ePlaywrightPlannedStepByIdDiagnosticLine(),
    '§21 playwright GUI e2e: npm run test:e2e:gui → planned-gui-e2e.spec.ts (8 skip без FLUXALLOY_E2E_APP; check:packaged-gui-e2e-playwright-deferred)',
