...
import { VELORIX_NEON_REFERENCE_SLIDESHOW_REL } from '../../../shared/velorix-neon-theme-tokens'
import { NeonReferenceOverlay } from '../components/NeonReferenceOverlay'
import { NeonWindowChrome } from '../components/NeonWindowChrome'

import { SS_ACTIVE_NAV, SS_STATUS_READY, SS_STATUS_ROWS } from './slideshow-ref15-data'
import { SlideshowUtilityRail, SlideshowWorkspace } from './slideshow-ref15-parts'
import { PROCESSING_NAV } from './processing-ref1-data'

/** ref.15 — Слайдшоу из изображений (mock; not sign-off). */
export function SlideshowScreen(): JSX.Element {
  return (
    <NeonWindowChrome>
      <div className="ss-shell" id="ref15" data-ref={VELORIX_NEON_REFERENCE_SLIDESHOW_REL}>
        {import.meta.env.DEV ? (
          <NeonReferenceOverlay referenceRel={VELORIX_NEON_REFERENCE_SLIDESHOW_REL} />
        ) : null}
            ...
            <nav className="processing-nav">
              {PROCESSING_NAV.map((item) => (
                <span
                  key={item.slug}
                  className={
                    item.slug === SS_ACTIVE_NAV
                      ? 'processing-nav__item processing-nav__item--active'
                      : 'processing-nav__item'
                  }
                  aria-current={item.slug === SS_ACTIVE_NAV ? 'page' : undefined}
                >
                  <span
