...
import { PL_ACTIVE_NAV, PL_STATUS_READY, PL_STATUS_ROWS } from './plugins-ref25-data'
import { PluginsCenter, PluginsDetailRail } from './plugins-ref25-parts'
import { PROCESSING_NAV } from './processing-ref1-data'

/** ref.25 — Плагины 3-col manager (mock; not sign-off). */
export function PluginsScreen(): JSX.Element {
  return (
    <NeonWindowChrome>
      <div className="pl-shell" id="ref25" data-ref={VELORIX_NEON_REFERENCE_PLUGINS_REL}>
        {import.meta.env.DEV ? (
        ...
        <aside className="tools-sidebar pl-sidebar" aria-label="Навигация">
            ...
            <nav className="processing-nav">
              {PROCESSING_NAV.map((item) => (
                <span
                  key={item.slug}
                  className={
                    item.slug === PL_ACTIVE_NAV
                      ? 'processing-nav__item processing-nav__item--active'
                      : 'processing-nav__item'
                  }
                  aria-current={item.slug === PL_ACTIVE_NAV ? 'page' : undefined}
                >
                  <span
          ...
          <div className="tools-sidebar__gpu vn-surface-glass">
            ...
            <p className="tools-sidebar__gpu-stats">Load 87% · Temp 71°C · VRAM 19.6 / 24.0 GB</p>
