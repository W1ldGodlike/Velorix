...
import { VELORIX_NEON_REFERENCE_SCENARIO_BUILDER_REL } from '../../../shared/velorix-neon-theme-tokens'
import { NeonReferenceOverlay } from '../components/NeonReferenceOverlay'
import { NeonWindowChrome } from '../components/NeonWindowChrome'

import { SB_ACTIVE_NAV, SB_STATUS_READY, SB_STATUS_ROWS } from './scenario-builder-ref16-data'
import { ScenarioBuilderWorkspace } from './scenario-builder-ref16-parts'
import { PROCESSING_NAV } from './processing-ref1-data'

/** ref.16 — Конструктор сценариев (mock; not sign-off). */
export function ScenarioBuilderScreen(): JSX.Element {
  return (
    <NeonWindowChrome>
      <div className="sb-shell" id="ref16" data-ref={VELORIX_NEON_REFERENCE_SCENARIO_BUILDER_REL}>
        {import.meta.env.DEV ? (
          <NeonReferenceOverlay referenceRel={VELORIX_NEON_REFERENCE_SCENARIO_BUILDER_REL} />
        ) : null}
            ...
            <nav className="processing-nav">
              {PROCESSING_NAV.map((item) => (
                <span
                  key={item.slug}
                  className={
                    item.slug === SB_ACTIVE_NAV
                      ? 'processing-nav__item processing-nav__item--active'
                      : 'processing-nav__item'
                  }
                  aria-current={item.slug === SB_ACTIVE_NAV ? 'page' : undefined}
                >
                  <span
