...
- `npm run check:packaged-gui-e2e-playwright-deferred` — §21 Playwright GUI e2e отложен: 8 `planned-gui-e2e`, зарезервирован `test:e2e:gui` (пока **нет** в `package.json`); канон — `packaged-gui-e2e-playwright-meta.ts`.
