...
После успешного завершения шага `pack:dir` в корне репозитория появится распакованное приложение **`dist/win-unpacked/`** для быстрого ручного smoke (см. §4). Сразу после этого **`npm run check:release`** вызывает **`npm run verify:win-unpacked`** — автоматическая проверка: `Velorix.exe`, непустые `resources/bin/{yt-dlp,ffmpeg,ffprobe}.exe`, `resources/VELORIX_TZ.md`, `resources/Data/trusted_hashes.json`, каталог `resources/Help/` (без запуска приложения). Пропуск: `VELORIX_SKIP_PACK_VERIFY=1`.
...
- §21 Playwright: `planned-gui-e2e.spec.ts` + `planned-gui-e2e-step-runners.ts` (skip без `VELORIX_E2E_APP`); `npm run test:e2e:gui` после `pack:dir`; приёмка на железе — `IMPLEMENTATION_MANUAL_VERIFICATION.md` **21.x**.
