...
| Main (исходник)                 | [`src/main/index.ts`](../src/main/index.ts) + [`src/main/ipc/`](../src/main/ipc/) + [`settings-ipc-persist.ts`](../src/main/settings-ipc-persist.ts)                                                     | Жизненный цикл `app`, окна; invoke IPC — `ipc/register-*`, `downloads-window`, `inspector-window`; persist theme/locale/ffmpeg/UI panels/engine paths — `createSettingsIpcPersist` + `MainSettingsAccess`; в `index.ts` — `ipcMain.on` (лог renderer) и оркестрация export/batch через `ExportBatchIpcHost`. |
