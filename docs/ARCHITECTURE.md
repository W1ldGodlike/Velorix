...
| Main (исходник)                 | [`src/main/index.ts`](../src/main/index.ts) + [`src/main/ipc/`](../src/main/ipc/) + [`preview-proxy-service.ts`](../src/main/preview-proxy-service.ts) + [`ytdlp-download-cli-merge.ts`](../src/main/ytdlp-download-cli-merge.ts) + [`main-application-menu.ts`](../src/main/main-application-menu.ts) + [`settings-ipc-persist.ts`](../src/main/settings-ipc-persist.ts) | Жизненный цикл `app`, окна; invoke IPC — `ipc/register-*`; меню приложения, preview proxy, merge CLI yt-dlp — отдельные модули; в `index.ts` — `ipcMain.on` и оркестрация export/batch. |
