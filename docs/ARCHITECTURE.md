...
- **`src/main/`** — сервисы: движки, очередь yt-dlp (`ytdlp-progress-parser.ts` + `ytdlp-progress-parser-download.ts` + `ytdlp-progress-parser-queue.ts`), ffprobe (`ffprobe-service.ts` + `ffprobe-track-detail-builder.ts` + `ffprobe-probe-json.ts`), одиночный и **пакетный** экспорт ffmpeg (`ffmpeg-export-service.ts` + `ffmpeg-export-app-settings-merge.ts` + `ffmpeg-export-spawn-once.ts`, `ffmpeg-export-batch-*`), терминал (`terminal-service`), медиа-протокол, настройки, логи, support bundle, база знаний (`knowledge-service`).
