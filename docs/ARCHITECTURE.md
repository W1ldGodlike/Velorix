...
| Main (исходник)                 | [`src/main/index.ts`](../src/main/index.ts) + [`src/main/ipc/`](../src/main/ipc/) + [`preview-proxy-service.ts`](../src/main/preview-proxy-service.ts) + [`ytdlp-download-cli-merge.ts`](../src/main/ytdlp-download-cli-merge.ts) + [`settings-ipc-persist.ts`](../src/main/settings-ipc-persist.ts) | Жизненный цикл `app`, окна; invoke IPC — `ipc/register-*`, `downloads-window`, `inspector-window`; WebM preview proxy и merge CLI yt-dlp — отдельные модули; persist — `createSettingsIpcPersist`; в `index.ts` — `ipcMain.on` (лог renderer) и оркестрация export/batch. |
