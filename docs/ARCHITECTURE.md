...
| Main (исходник)                 | [`src/main/index.ts`](../src/main/index.ts) + [`src/main/ipc/`](../src/main/ipc/)                                                                                                                     | Жизненный цикл `app`, окна; invoke IPC — `ipc/register-*`, `downloads-window`, `inspector-window`; в `index.ts` остаётся `ipcMain.on` (лог renderer) и оркестрация export/batch через `ExportBatchIpcHost`.                                                                              |
