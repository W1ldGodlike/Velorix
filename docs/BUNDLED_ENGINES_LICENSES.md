...
Velorix может поставляться с runtime-движками в `resources/bin` (исходно `bin/` перед упаковкой). Эти файлы не являются частью исходного кода Velorix и имеют собственные лицензии.
...
4. Optional: tune download timeouts via `VELORIX_ENGINE_DOWNLOAD_TIMEOUT_MS` (see `docs/RELEASE.md`).
