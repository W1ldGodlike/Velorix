# [УСТАРЕЛО] Velorix v0 UX Reference

> **Не использовать для новой работы.** Канон визуала и UI-ориентир с **2026-05-22**: [`VELORIX_NEON_THEME.md`](VELORIX_NEON_THEME.md), эталонные PNG — каталог [`reference/`](reference/).  
> Файл сохранён как историческая запись обхода `v0.vercel.app` (журнал J-138–J-214). Ссылки из кода и rules на `UX_REFERENCE_V0.md` удалены (**J-1620**).

---

Краткая карта v0-референса для Cursor SDK/Assistant (архив). Раньше — ориентир для UI/UX-переноса без слепого копирования.
...
- Settings rail sections:
  ...
  - `Сохранение`;
  - `Сеть`;
  - `Эксперт и превью` (доп. argv, селектор токена по категориям, превью команды; длинный справочник — в pop-out).
