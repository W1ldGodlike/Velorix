...
describe('COMMAND_PALETTE_ITEMS', () => {
  it('includes NEON export/seek actions', () => {
    ...
    expect(ids.has('open-help')).toBe(true)
    ...
    expect(ids.has('batch-export-start')).toBe(true)
    expect(ids.has('start-preview-export')).toBe(true)
    expect(ids.has('knowledge-session-queues')).toBe(true)
    expect(ids.has('dev-showcase-ref27')).toBe(true)
    expect(ids.has('dev-showcase-ref26')).toBe(true)
    expect(ids.has('dev-showcase-ref1')).toBe(true)
    expect(ids.has('toggle-preview-play')).toBe(true)
    expect(ids.has('knowledge-processing-history')).toBe(true)
    expect(ids.has('seek-preview-start')).toBe(true)
    expect(ids.has('seek-preview-end')).toBe(true)
    expect(ids.has('timeline-zoom-out')).toBe(true)
    expect(ids.has('timeline-zoom-reset')).toBe(true)
    expect(ids.has('timeline-zoom-in')).toBe(true)
    expect(ids.has('set-export-trim-in')).toBe(true)
    expect(ids.has('set-export-trim-out')).toBe(true)
  })
})
