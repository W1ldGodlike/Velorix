...
### 5.1 Тема (тёмная / светлая / как в системе)

- [ ] **5.1.1** Primary-кнопки и accent-ссылки читаемы (особенно тёмная тема).
- [ ] **5.1.2** Focus Tab на полях, select и кнопках.
- [ ] **5.1.3** Disabled приглушены, но различимы.
- [ ] **5.1.4** Модалки «Настройки» и «О программе» — backdrop без грязного чёрного.
- [ ] **5.1.5** Сервис → конструктор/планировщик — узлы и подписи на токенах.
- [ ] **5.1.6** Pop-out «Загрузки» — та же тема, что главное окно.
- [ ] **5.1.7** Окно инспектора — фон и topbar согласованы с редактором.
- [ ] **5.1.8** Режим «как в системе»: смена темы ОС — приложение следует без перезапуска.
...
- [ ] **19.4.1** `npm run dev` (Win): pop-out `#downloads` и `#inspector` — тема, очередь, инспектор (главное окно — J-1454).
