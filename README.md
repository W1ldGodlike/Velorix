...
- **Vitest (Windows gate):** **280** test files, **1901** tests — снимок [`IMPLEMENTATION_CHECKLIST.md`](./IMPLEMENTATION_CHECKLIST.md); lock [`toolchain-baseline-governance.test.ts`](./tests/shared/toolchain-baseline-governance.test.ts).
- **Peer dependencies:** в корне [`.npmrc`](./.npmrc) уже `legacy-peer-deps=true` (Vite 8 / `electron-vite`); при ручном install без `.npmrc` — `npm install --legacy-peer-deps` ([план toolchain](./docs/TOOLCHAIN_BASELINE_UPGRADE_PLAN.md)).
- **Dependabot (wave 5):** после того как baseline toolchain оказался на **`main`** (у мейнтейнера после `git push`), закрыть устаревшие major-PR через **`gh`** — сценарий в [плане toolchain §Git](./docs/TOOLCHAIN_BASELINE_UPGRADE_PLAN.md); предрелиз и операционно — [`docs/RELEASE.md`](./docs/RELEASE.md) §1 и [`docs/AGENT_OPERATIONAL_NOTES.md`](./docs/AGENT_OPERATIONAL_NOTES.md).
...
- **Агент Cursor:** [`AGENTS.md`](./AGENTS.md) → [`docs/SOURCES_OF_TRUTH.md`](./docs/SOURCES_OF_TRUTH.md); WIP (**27**+ paths, gate **J-1440**) — [`docs/AGENT_MARATHON.md`](./docs/AGENT_MARATHON.md) §Pre-commit; **Следующий cadence** **J-1560** (commit); WIP push — по «push» (§Pre-commit; J-1440 push отложен).
...
- Help §21: `npm run check:help-workflow-smoke-crosslinks` (44 workflow; tail 42 HelpCrosslinksCountTail + ffmpeg FfmpegTerminalWorkflowClause + knowledge KnowledgeHubDevClause (FAQ 2 in tail, outside 44); 44/44 strict packaged-smoke formatters (`STRICT_PACKAGED_SMOKE_HELP_PATHS`)); registry `check:help-smoke-guards-package-json` requires `partition:` in all 44 workflow Help.
