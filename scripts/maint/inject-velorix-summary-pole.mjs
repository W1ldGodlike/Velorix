...
/**
 * Adds "(поле …)" gloss inside summary strings that mention velorix-ytdlp-*.txt
 * when missing. Does not change fullLine. Safe to run repeatedly.
 *
 * Usage: node scripts/inject-velorix-summary-pole.mjs
 *        npm run locales:terminal-velorix-pole
 *
...
export function glossVelorixPrintToFileSummary(inner) {
  if (!inner.includes('velorix-ytdlp') || inner.includes('(поле ')) return inner
  let t = inner
  ...
  t = t.replace(/\((tbr), kbps\) в (velorix-ytdlp)/g, '(поле $1, kbps) в $2')
  t = t.replace(/\(([a-z][a-z0-9_]*)\) в (velorix-ytdlp)/gi, '(поле $1) в $2')
  t = t.replace(
  ...
  t = t.replace(
    /^Записать (\w+) в отдельный (velorix-ytdlp[^\s;]+)/,
  ...
  if (!/^Записать \(поле /.test(t)) {
    t = t.replace(/^Записать ([a-z][a-z0-9_]*) в (velorix-ytdlp)/i, 'Записать (поле $1) в $2')
  }
...
const ranAsCli = process.argv[1]?.replace(/\\/g, '/').endsWith('inject-velorix-summary-pole.mjs')
if (ranAsCli) main()
