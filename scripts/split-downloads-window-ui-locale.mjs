...
const lines = execSync('git show HEAD:src/shared/downloads-window-ui-locale.ts', {
  encoding: 'utf8'
}).split(/\r?\n/)
...
fs.writeFileSync(
  path.join(outDir, 'downloads-window-ui-locale-types.ts'),
  `${typesBlock}
`
)
...
import type { DownloadsWindowUiLocale } from './downloads-window-ui-locale-types'
...
fs.writeFileSync(path.join(outDir, 'downloads-window-ui-locale.ts'), entry)
console.log('split downloads-window-ui-locale OK')
