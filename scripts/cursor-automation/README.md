...
Промпты по умолчанию: каталог **`prompts/`** (`initial.txt`, `continue.txt`). Общие правила поведения — **`prompts/agent-contract.txt`**; именно туда добавляйте нюансы, которые агент должен соблюдать всегда. Текущий главный приоритет SDK закреплён там же: сначала v0/UI по `docs/UX_REFERENCE_V0.md`, потом остальные задачи проекта. Свой текст — через `PROMPTS_DIR` или правка файлов.
...
Файл **`STOP`** в этой папке (`scripts/cursor-automation/STOP`) работает как флаг:
