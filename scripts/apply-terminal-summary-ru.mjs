...
const pairs = [
  ...
  ['JSON метаданных только для одного ролика из URL-плейлиста', 'JSON метаданных только для одного ролика по ссылке на плейлист'],
  ['Сухой прогон одного ролика из URL-плейлиста', 'Сухой прогон одного ролика по ссылке на плейлист'],
  ['Только один ролик из URL-плейлиста и заголовок', 'Только один ролик по ссылке на плейлист и заголовок'],
  ...
  ['Список имён модулей извлечения сайтов без URL', 'Список имён модулей извлечения сайтов в выводе; ссылка в команде не нужна'],
  ['Версия yt-dlp и зависимостей без URL', 'Версия yt-dlp и зависимостей в выводе; ссылка в команде не нужна'],
  ...
  ['Плоский плейлист: URL каждого элемента без глубокого извлечения', 'Плоский плейлист: ссылка каждого элемента без глубокого извлечения'],
  ...
  ['Плоский плейлист: URL страницы каждого элемента без глубокого извлечения', 'Плоский плейлист: ссылка на страницу каждого элемента без глубокого извлечения'],
  ...
  ['Плоский плейлист: все URL элементов без скачивания', 'Плоский плейлист: все ссылки элементов без скачивания'],
  ['Разрешить file:// URL в аргументах', 'Разрешить ссылки file:// в аргументах'],
  ['не похож на URL', 'не похож на ссылку'],
  ['Проверить все HTTP-URL фрагментов перед загрузкой', 'Проверить все HTTP-ссылки фрагментов перед загрузкой'],
  ['Имена и краткие описания модулей извлечения (extractors) без URL', 'Имена и краткие описания модулей извлечения (extractors) в выводе; ссылка в команде не нужна'],
  ['Плейлист: продолжать после ошибки отдельного URL', 'Плейлист: продолжать после ошибки отдельной ссылки'],
  ...
  ['(канонический URL вкладки или канала плейлиста', '(каноническая ссылка на вкладку или канал плейлиста'],
  ...
  ['диагностика, ZIP-архив для обращения в поддержку', 'диагностика, архив ZIP для обращения в поддержку'],
  ['трассировка для ZIP-архива поддержки', 'трассировка для обращения в поддержку в виде архива ZIP'],
  ...
  ['(discontinuity, --hls-split-discontinuity', '(метка разрыва discontinuity в плейлисте, --hls-split-discontinuity'],
  ['probe_score контейнера (насколько уверенно выбран демультиплексор)', 'Поле probe_score по контейнеру: насколько уверенно выбран демультиплексор'],
  ['Контейнер: probe_score (уверенность ffprobe в формате)', 'Контейнер: поле probe_score (уверенность ffprobe в формате)'],
  [
  ...
  ['Тег языка первой аудиодорожки (stream_tags: language)', 'Тег языка первой аудиодорожки (теги дорожки stream_tags: language)'],
  ...
  ['MKV-статистика v:0: stream_tags BPS и DURATION', 'MKV-статистика v:0: теги дорожки stream_tags BPS и DURATION'],
  ['(поле side_data_list (боковые данные): матрица', '(поле side_data_list: матрица'],
  ['(-af ebur128=framelog=verbose);', '(-af ebur128=framelog=verbose — подробный журнал по кадрам);'],
  [
