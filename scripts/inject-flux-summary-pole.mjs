...
export function glossFluxPrintToFileSummary(inner) {
  ...
  t = t.replace(/^Записать fps выбранного формата в /, 'Записать частоту кадров (поле fps) выбранного формата в ')
  t = t.replace(
  ...
  t = t.replace(/^Записать словарь миниатюр \(thumbnails/i, 'Записать словарь миниатюр (поле thumbnails;')
  t = t.replace(
