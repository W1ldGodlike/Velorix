...
- **`releaseSmoke:`** — CI packaged pipeline (`smoke:packaged-release`), §21 e2e registry (ci-headless / planned-gui-e2e / manual-owner, per-step `e2e <id>:`, `planned GUI e2e scope` line), **present/missing** layout for `dist/win-unpacked/`, `dist/linux-unpacked/`, `FluxAlloy.app`; dev: `check:packaged-e2e-scenarios-registry`, `check:help-workflow-smoke-crosslinks`.
