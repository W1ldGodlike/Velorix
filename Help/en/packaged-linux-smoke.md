...
**Copy** in the packaged panel matches Support ZIP (`owner:` / `step [id]:`). English UI — `locales/en/linux-packaged-manual-smoke.json`. Full owner bundle — [owner-manual-smoke.md](owner-manual-smoke.md). Dev: `check:packaged-manual-smoke-parity`, `check:packaged-e2e-scenarios-registry` (§21; per-step `e2e <id>:` in `releaseSmoke:`, e.g. `e2e launch:`) — in `check:quiet`.
