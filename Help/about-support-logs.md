...
- **`releaseSmoke:`** — CI packaged pipeline (`smoke:packaged-release`), реестр §21 e2e (в т.ч. per-step `e2e <id>:`), layout **present/missing** для `dist/win-unpacked/`, `dist/linux-unpacked/`, `FluxAlloy.app` (без повторного `verify:*` на другой ОС); dev: `check:packaged-e2e-scenarios-registry`.
